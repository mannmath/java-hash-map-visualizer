...
function getTextForStep(step: number): string {
  switch (step) {
    ...
    case 1:
      return "Ok. We can see a node has been added to the same bucket. As per our earlier discussion, this is now a collision scenario(we talked about it before...remember?). So what do we do? Nothing really, elements will continue to be added to the buckets in a <a href='https://www.geeksforgeeks.org/data-structures/linked-list/' target='_blank'>linked list</a> fashion. This strategy is helpful, incase of a collision, i.e., when multiple entries(key-value pairs) of our hash map try to occupy the same bucket, because hash value of their keys happened to be the same. Cool right? Now with that out of the way, let's move on to the next item on our inventory.";
